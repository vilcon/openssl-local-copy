...
int req_main(int argc, char **argv)
{
 ...
 opthelp:
    ...
    if (extsect != NULL) {
        ...
        if (!do_EXT_add_nconf(req_conf, &ctx, extsect, NULL,
                              !gen_x509
    ...
    if (addext_conf != NULL) {
        ...
        X509V3_set_ctx_test(&ctx);
        if (!do_EXT_add_nconf(addext_conf, &ctx, NULL, NULL,
 ...
 loop:
    ...
    if (newreq || gen_x509) {
        ...
        if (gen_x509) {
            ...
            /* Add extensions */
            if (extsect != NULL
                && !do_EXT_add_nconf(req_conf, &ext_ctx, extsect, new_x509,
                                     "Error adding x509 extensions from section %s\n"))
                goto end;
            if (addext_conf != NULL
                && !do_EXT_add_nconf(addext_conf, &ext_ctx, NULL, new_x509,
            ...
            /* Add extensions */
            if (extsect != NULL
                && !do_EXT_REQ_add_nconf(req_conf, &ext_ctx, extsect, req,
                                         "Error adding request extensions from section %s\n"))
                goto end;
            if (addext_conf != NULL
                && !do_EXT_REQ_add_nconf(addext_conf, &ext_ctx, "default", req,

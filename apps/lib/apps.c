...
int do_EXT_add_nconf(CONF *conf, X509V3_CTX *ctx, const char *sect,
                     X509 *cert, const char *msg)
{
    if (X509V3_set_nconf(ctx, conf)
            && X509V3_EXT_add_nconf(conf, ctx, sect != NULL ? sect : "default",
                                    cert))
        return 1;
...
int do_EXT_REQ_add_nconf(CONF *conf, X509V3_CTX *ctx, const char *sect,
                         X509_REQ *req, const char *msg)
{
    if (X509V3_set_nconf(ctx, conf)
            && X509V3_EXT_REQ_add_nconf(conf, ctx, sect, req))
        return 1;
    BIO_printf(bio_err, msg, sect);
    return 0;
}
